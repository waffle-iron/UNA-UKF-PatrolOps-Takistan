...

{
	...
	,"PO3_loc_military_6","PO3_loc_military_7","PO3_loc_military_8","PO3_loc_military_9"
];
...

{
	_position = _x call PO3_fnc_getPos;
	if(_position distance [0,0,0] > 10) then {
		_location = createLocation ["NameLocal", _position, 400, 400];
		_location setText "Urban Centre";
		_location setName "Urban Centre";
	};
}forEach ["rip_urban_1","rip_urban_2","rip_urban_3","rip_urban_4","rip_urban_5"];
...

{
	...
	if(_position distance [0,0,0] > 10) then {
		_location = createLocation ["NameLocal", _position, 100, 100];
		_location setText "RIP";
		_location setName "RIP";
	};
}forEach ["rip_POI_1","rip_POI_2","rip_POI_3","rip_POI_4","rip_POI_5","rip_POI_6","rip_POI_7","rip_POI_8","rip_POI_9","rip_POI_10","rip_POI_11","rip_POI_12","rip_POI_13","rip_POI_14","rip_POI_15","rip_POI_16","rip_POI_17"
];
		...
		_text = switch (toLower (type _x)) do {
			...
			case "namelocal"		: {
				switch ( toLower(text _x) ) do {
					...
					case "rip"				: { _locPOI	set [count _locPOI,[_x,"ColorIndependent"]]; "Mine" };
					case "military post"	: { _locBSE set [count _locBSE,[_x,"ColorOPFOR"]]; "Military" };
					case "urban centre"		: { _locCTY	set [count _locCTY,[_x,"ColorOrange"]]; "City" };
