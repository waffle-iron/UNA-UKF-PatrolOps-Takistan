...
_ehF = _vehicle addEventHandler ["Fired",{
	if(PO3_protector_enable) then {
		_p = _this select 6;
		if(
			_p distance (getMarkerPos "respawn_west") < 1 ||
			_p distance (getMarkerPos "respawn_east") < 1 ||
			_p distance (getMarkerPos "respawn_guerrila") < 1
		)then {
			...
			_p spawn {
				waitUntil {
					if(
						_this distance (getMarkerPos "respawn_west") < 1 ||
						_this distance (getMarkerPos "respawn_east") < 1 ||
						_this distance (getMarkerPos "respawn_guerrila") < 1
						/* Was 250m */
					)then {
						[player,"HINTC",localize "STR_PO3_DIALOG_CEASEFIRE"] call PO3_fnc_hint;
						deleteVehicle _this;
